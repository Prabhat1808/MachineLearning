...
compute_score()
{
    ...
    if [ -f "$3" ]; then
        ...
        echo -e "Accuracy on dummy target values is $score ."
...
evaluate()
{
    ...
    if [ $status == "OK" ]; then
        part="a"
        echo -e "\nEvaluating SVM part(a)"
        run "$time_a" "$logs" "$fname" "$part" "$data_folder_path/DHC_train.csv" "$data_folder_path/DHC_test.csv" "$stud_folder_path/predictions_svm_${part}" 10  
        compute_score "$compute_accuracy" "$data_folder_path/dummy_DHC_target_labels.txt" "$stud_folder_path/predictions_svm_${part}.txt" "$stud_folder_path/result_svm_${part}.txt"         

        part="b"
        echo -e "\nEvaluating SVM part(b)"
        run "$time_b" "$logs" "$fname" "$part" "$data_folder_path/DHC_train.csv" "$data_folder_path/DHC_test.csv" "$stud_folder_path/predictions_svm_${part}" 10 0.01
        compute_score "$compute_accuracy" "$data_folder_path/dummy_DHC_target_labels.txt" "$stud_folder_path/predictions_svm_${part}.txt" "$stud_folder_path/result_svm_${part}.txt" 
        
        part="c"
        echo -e "\nEvaluating SVM part(c)"
        run "$time_c" "$logs" "$fname" "$part" "$data_folder_path/DHC_train.csv" "$data_folder_path/DHC_test.csv" "$stud_folder_path/predictions_pca_svm_${part}" 10 0.01
        compute_score "$compute_accuracy" "$data_folder_path/dummy_DHC_target_labels.txt" "$stud_folder_path/predictions_pca_svm_${part}.txt" "$stud_folder_path/result_svm_${part}.txt" 
        echo -n $'\n' >> "$score_file"
...
sandbox_dir="$3"
submissions_dir="$4"
#write_score "ENTRY_NUM,PEN_BASH,TIME_a,Acc_a,TIME_b,Acc_b,TIME_c,Acc_c"
