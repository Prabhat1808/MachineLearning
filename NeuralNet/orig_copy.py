...
class neural_net():
	...
	def train(self,training_data,epochs,mini_batch_size,eta,evaluation_data=None,monitor_evaluation_cost=False,monitor_evaluation_accuracy=False,monitor_training_cost=False,monitor_training_accuracy=False):
		...
		for j in range(epochs):
			...
			for i in range(batches-1):
				...
				y_tr = y_train[i*mini_batch_size:(i+1)*mini_batch_size,:]
				# print ("Iteration :", i)
				curr_loss += self.gradient_descent((x_tr,y_tr), eta_eff) # check this
			...
			# diff = acts[-1] - y_train.transpose()
			# nor = np.sum((np.linalg.norm(diff,axis=1))**2) / n

			# # curr_loss = self.total_cost(list(zip([x.reshape((784,1)) for x in x_train],[y.reshape((46,1)) for y in y_train])))
			# curr_loss = nor
			curr_loss /= n
			if(curr_loss > prev_loss):
				k += 1
	...
	def gradient_descent(self, batch, eta):
		...
		derivs,err = self.backpropagation(x_tr,y_tr)
		db = derivs[0]
		dw = derivs[1]
		# for i in range(n):
			# x = x_tr[i]
...
net = neural_net(784,[200],46,activation = 'tanh')
