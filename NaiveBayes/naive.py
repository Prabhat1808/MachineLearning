...
def train_model_tfidf(data,vocab,inv_vocab,n_grams=1):
	...
	wc = np.log(len(labels)/ (np.sum(wc,axis=0) + 1))
	tf = (tf.multiply(wc)).tocsr()
	# print (type(tf))
...
# 		vocab = create_vocabulary(train_n['reviews'],n_grams=1)
# 		inv_vocab = invert_vocab(vocab)
# 		model = train_model_freq(train_n,vocab,inv_vocab,n_grams=1)
# 		predictions = [int(predict_freq(rev,model,vocab,inv_vocab,n_grams=1)) for rev in test_n["reviews"]]
# 		print (np.bincount(predictions))
		# np.savetxt("test_preds_freq/"+str(i)+""+str(k)+""+str(j)+".txt",predictions,fmt="%i")
